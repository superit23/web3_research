...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        (uint256 dvtIdx, uint256 playerDVTAmount)   = getPlayerReward("/test/the-rewarder/dvt-distribution.json");
        (uint256 wethIdx, uint256 playerWETHAmount) = getPlayerReward("/test/the-rewarder/weth-distribution.json");
        ...
        uint256 numDVTTokens  = (distributor.getRemaining(address(dvt)) / playerDVTAmount);
        uint256 numWETHTokens = (distributor.getRemaining(address(weth)) / playerWETHAmount);

        // Create Alice's claims
        Claim[] memory claims = new Claim[](numDVTTokens + numWETHTokens);
        ...
        Claim memory dvtClaim = Claim({
            ...
            tokenIndex: 0, // claim corresponds to first token in `tokensToClaim` array
            proof: merkle.getProof(dvtLeaves, dvtIdx) // Alice's address is at index 2
        });
        ...
        Claim memory wethClaim = Claim({
            ...
            tokenIndex: 1, // claim corresponds to second token in `tokensToClaim` array
            proof: merkle.getProof(wethLeaves, wethIdx) // Alice's address is at index 2
        });

        for(uint256 i=0; i < numDVTTokens; i++) {
            claims[i] = dvtClaim;
        }

        for(uint256 i=numDVTTokens; i < (numWETHTokens + numDVTTokens); i++) {
            claims[i] = wethClaim;
        }
    ...
    function getPlayerReward(string memory path) public view returns (uint256, uint256) {
        Reward[] memory rewards = abi.decode(vm.parseJson(vm.readFile(string.concat(vm.projectRoot(), path))), (Reward[]));
        Reward memory playerReward;
        uint256 index;

        for (uint256 i=0; i < rewards.length; i++) {
            if (rewards[i].beneficiary == player) {
                playerReward = rewards[i];
                index = i;
            }
        }

        return (index, playerReward.amount);
    }
}
